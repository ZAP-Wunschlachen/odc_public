...
for iteration in range(2):
    ...
    assert all(edge.is_manifold for edge in bm.edges), 'Pontic has open or nonmanifold edges'
    assert all(math.isfinite(value) for vertex in bm.verts for value in vertex.co)
    ...
    assert all(edge.is_manifold for edge in bm.edges), 'Evaluated pontic must be closed'
    assert all(math.isfinite(value) for vertex in bm.verts for value in vertex.co)
    assert abs(bm.calc_volume()) > 0
    bm.free()
...
assert tooth.margin == ''
assert tooth.log == 'preserve treatment notes' and tooth.rest_type == '1'
...
artifact = ROOT / 'tests/artifacts/pontic_import.blend'
...
print('ODC_PONTIC_IMPORT_PASSED', bpy.app.version_string)
