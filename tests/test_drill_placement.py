...
asset = u.obj_list_from_lib(u.get_settings().drill_lib, include='Drill', exclude='Sleeve')[0]
old_name = None
count = len(bpy.data.objects)
for depth in (-2, 5):
    assert bpy.ops.opendental.place_drill(drill=asset, depth=depth) == {'FINISHED'}
    sleeve = bpy.data.objects[space.drill]
    bpy.context.view_layer.update()
...
space.implant = ''
assert bpy.ops.opendental.place_drill(drill=asset) == {'CANCELLED'}
assert bpy.data.objects.get(old_name) is not None
addon_utils.disable(ROOT.name, default_set=True)
print('ODC_DRILL_PLACEMENT_PASSED', bpy.app.version_string)
