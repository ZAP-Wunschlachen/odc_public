...
def run():
    try:
        ...
        module=importlib.import_module(f'{ROOT.name}.Operators.model_ops')
        module.ShowMessageBox=lambda **kwargs:None
        ...
        bpy.context.scene.ODC_modops_props.show_box=True
        area=next(a for a in bpy.context.screen.areas if a.type=='VIEW_3D')
        region=next(r for r in area.regions if r.type=='WINDOW')
        area.spaces.active.region_3d.view_rotation=rotation
        with bpy.context.temp_override(area=area,region=region):
            assert bpy.ops.opendental.solid_hollow_models()=={'FINISHED'}
        assert bpy.context.scene.ODC_modops_props.base_height==3
        assert bpy.context.scene.ODC_modops_props.show_box
        outputs=[o for o in bpy.context.scene.objects if o!=source]
        ...
        for output in outputs:
            bm=bmesh.new();bm.from_mesh(output.data)
            assert bm.faces and all(e.is_manifold for e in bm.edges),output.name
            bm.free()
            local=[rotation.inverted() @ (output.matrix_world@v.co) for v in output.data.vertices]
            assert abs(min(p.z for p in local)+3)<1e-4,(output.name,min(p.z for p in local))
        print('ODC_SOLID_HOLLOW_MODELS_PASSED',flush=True)
